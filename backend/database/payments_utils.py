...
from database.models import Payment, Contract, AuditLog
...
# تحقق من البيانات (مثال: عدم التكرار لنفس العقد بنفس التاريخ)
...
# إضافة دفعة جديدة
def add_payment(db: Session, contract_id, due_date, amount_due, amount_paid=0.0, paid_on=None, is_late=False, notes=None):
    validate_contract(db, contract_id)
    check_payment_conflict(db, contract_id, due_date)
    payment = Payment(
    ...
    db.commit()
    db.refresh(payment)
    log_audit(db, user="system", action="add", table_name="payments", row_id=payment.id, details=f"Add: contract_id={contract_id}, due={due_date}")
    return payment
...
def list_payments(db: Session, page=1, per_page=20, filter_contract_id=None, filter_is_late=None):
...
def export_payments_to_csv(db: Session):
    ...
    writer.writerow(['id', 'contract_id', 'due_date', 'amount_due', 'amount_paid', 'paid_on', 'is_late', 'notes'])
    for p in payments:
        writer.writerow([
            p.id,
            p.contract_id,
